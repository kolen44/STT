...
class VADConfig:
    ...
    # Адаптивные паузы (как в ChatGPT - НЕ 6 секунд!)
    MIN_PAUSE_MS = 500        # Минимальная пауза для коротких фраз ("да", "нет")
    DEFAULT_PAUSE_MS = 700    # Стандартная пауза для обычных фраз
    MAX_PAUSE_MS = 1200       # Максимальная пауза для длинных предложений
    QUESTION_PAUSE_MS = 600   # Для вопросов нужна меньшая пауза
